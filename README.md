...
[![Tests](https://img.shields.io/badge/tests-552%20passing-success)](CHECKPOINT.md)
...
### Scientific Evidence Freeze: The Four-Level Evidence Taxonomy

Under the official **Scientific Evidence Freeze** ([`artifacts/evaluation/evidence_freeze/`](artifacts/evaluation/evidence_freeze/)), all 18 named RAI capabilities are formally classified into exactly one of four tiers. The full 10-field record for every capability — evidence source, dataset/fixture, real-vs-synthetic, external-vs-internal, metric/result, artifact, what the evidence proves, what it does NOT prove — is in [`evidence_registry.csv`/`.json`](artifacts/evaluation/evidence_freeze/evidence_registry.csv); the judge-readable summary is [`freeze_summary.md`](artifacts/evaluation/evidence_freeze/freeze_summary.md).

| Level | Definition | Capabilities in RAI |
|---|---|---|
| **1. VALIDATED** | Measured against an external dataset or peer-reviewed public benchmark | **CARE Wind Anomaly Detection** (Farms A/B/C, Zenodo 10958775; normal acc > 0.995)<br>**CARE Cross-Farm Transfer** (Gate 5.4 target-normal calibration, 106.3% recovery)<br>**Kelmarsh Benchmark** (Zenodo 5841834; operational event window association — NOT failure prediction)<br>**Solar PVDAQ Data Foundation** (NREL PVDAQ OEDI 450 daily files; Gate 5.6A/B data acquisition only) |
| **2. DEMONSTRATED** | Fully implemented and functional end-to-end, but demonstrated using controlled, synthetic, internal, or self-graded fixtures rather than an independent external benchmark | **Environmental Context / Discrimination** (real weather inputs, internal-simulator fault ground truth)<br>**Historical Case Retrieval** (real 14-case corpus, but P@1/R@3 self-graded against developer-authored relevance judgments)<br>**RAG** (FTS5/BM25 over 19 curated internal documents)<br>**Local AI Agent Reasoning** (Tasks A-G on internal synthetic battery; 0.00% unsupported claims)<br>**Economic Consequence Analysis** (NPV comparison under stated cost assumptions)<br>**Work Orders** (Next.js `/work-orders` console and approval workflow)<br>**Technician Feedback** (capture + dual-key gate; 0 tickets promoted to `EXTERNAL_REAL`)<br>**Closed-Loop Learning** (feedback-to-retrieval ingestion, dual-key promotion gate)<br>**Frontend Operational Workflow** (clean build/lint, full browser-verified workflow) |
| **3. ARCHITECTURALLY SUPPORTED** | Implemented and technically bounded, but not validated against real-world outcomes — only against its own invariants | **Diagnosis** (counterevidence competing-hypothesis elimination)<br>**Recommendation Engine** (confidence-gated policy, deterministic abstention)<br>**Dispatch Optimization** (heuristic wind <12m/s & rain 0mm window scheduler)<br>**Weather-Aware Scheduling** (same engine's forecast-gating facet) |
| **4. NOT VALIDATED** | Explicitly not supported by current evidence — must NOT be claimed | **Solar Physics Layer** (Gate 5.6C remains `NOT_INDEPENDENTLY_VALIDATED` — Gate 5.6B's Validation cohort is empty)<br>**Real-Time Utility Deployment** (zero live commercial plant telemetry feeds connected)<br>**Autonomous Self-Learning** (demo/test tickets remain quarantined from `EXTERNAL_REAL`) |
...
narrative layered on top of it — every box above is a module you can open and read. **The last
stage is an active, verified closed loop:** `record_feedback` in `rai/memory/work_orders.py`
records technician inspection findings to `artifacts/tickets.jsonl`. Confirmed faults
and prevented failures are dynamically transformed into `Case` objects by
`get_field_feedback_cases()` in `rai/memory/library.py`. Under strict case promotion rules
(`docs/evaluation/CLOSED_LOOP_INTEGRITY_AUDIT.md`), only genuine field observations
(`EXTERNAL_FIELD_OBSERVED` + `PHYSICAL_INSPECTION_VERIFIED`) can be promoted to `EXTERNAL_REAL`,
guaranteeing zero retrieval contamination of the real benchmark partition, while automated test fixtures
and demo records remain safely quarantined as `INTERNAL_SYNTHETIC`. This verifies the complete
feedback-to-retrieval closed loop (`tests/test_closed_loop_learning.py`). Fleet dispatch and
meteorological windows are optimized via `rai/decision/dispatch_optimizer.py` under configured
operational constraints and surfaced in the Next.js Operations Console (`/work-orders`).
...
| **Historical Trajectory Memory & Real Corpus** | Audited external real cases (CARE to Compare Zenodo 14006163, Kelmarsh Zenodo 5841834, NREL PVDAQ) with strict event classification (`REAL_VERIFIED_EVENT` vs operational), adjudication proofs, trajectory embeddings, explainability breakdown (`why_matched`, `what_is_different`, `why_may_not_apply`), and abstention gating | `rai/memory/` (`real_corpus.py`, `retrieval.py`, `library.py`) |
...
├── memory/             Audited real-world case corpus (CARE, Kelmarsh, PVDAQ) & trajectory retrieval
