...
Under the official **Scientific Evidence Freeze** ([`artifacts/evaluation/evidence_freeze/`](artifacts/evaluation/evidence_freeze/)), every capability in RAI is formally classified into exactly one of four tiers:

| Level | Definition | Capabilities in RAI |
|---|---|---|
| **1. VALIDATED** | Measured against an external dataset or peer-reviewed public benchmark | **Wind Anomaly Detection** (CARE Farms A/B/C, Zenodo 10958775; normal acc > 0.995)<br>**Cross-Farm Wind Transfer** (Gate 5.4 target-normal calibration, 106.3% recovery)<br>**Solar Data Foundation** (NREL PVDAQ OEDI 450 daily files; Gate 5.6A/B)<br>**Kelmarsh Event Association** (Zenodo 5841834; operational event window correlation)<br>**Historical Case Retrieval** (14 audited external cases; P@1 90.0%, R@3 85.0%) |
| **2. DEMONSTRATED** | Fully implemented and functional end-to-end, but demonstrated using controlled/synthetic fixtures | **Simulator Physics Agreement** (12 injected failure scenarios in `rai/sim/`)<br>**Local AI Agent Reasoning** (Tasks A-G on internal synthetic battery; 0.00% unsupported claims)<br>**Techno-Economic Decision Support** (NPV comparison under stated cost assumptions)<br>**Work Order Lifecycle** (Next.js `/work-orders` console and approval workflow)<br>**Closed-Loop Ingestion** (Feedback-to-retrieval ingestion with dual-key promotion gate) |
| **3. ARCHITECTURALLY SUPPORTED** | Implemented and technically bounded, but not externally validated against real field outcomes | **Differential Diagnosis Engine** (Counterevidence competing hypothesis elimination)<br>**Safe-Weather Crew Dispatch** (Heuristic wind &lt;12m/s &amp; rain 0mm window scheduler) |
| **4. NOT VALIDATED** | Explicitly not supported by current evidence — must NOT be claimed | **Independent Solar Model Validation** (Gate 5.6C remains `NOT_INDEPENDENTLY_VALIDATED`)<br>**Kelmarsh Failure Prediction** (Operational status only; no hardware failure ground truth)<br>**Real-Time Utility Deployment** (Zero live commercial plant telemetry feeds connected)<br>**Autonomous Self-Learning** (Demo/test tickets remain quarantined from `EXTERNAL_REAL`) |
