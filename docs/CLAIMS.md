...
| Historical cases provide contextual evidence with preserved provenance and match explanations | **Partial, bounded retrieval capability.** The current corpus is internally authored synthetic cases; retrieval is not diagnosis or proof | `rai/memory/retrieval.py`, `rai/schemas.py`, `tests/test_historical_intelligence.py` | `.venv\\Scripts\\python.exe -m pytest tests/test_historical_intelligence.py -q` |
